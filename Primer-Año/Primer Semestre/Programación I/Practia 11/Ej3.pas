...
Procedure EliminarNodo (Var L:Lista; Var act,ant:Lista);
Begin
  If (act = L) Then
    L := act^.sig
  Else
...
Procedure RecorrerLista (Var L:Lista;Var V:vectorcontador; Var VecIns:
                         vectorinscriptos);
...
Begin
  ...
  While (aux <> Nil) Do
    Begin
      If (aux^.datos.presento = 'no' ) Then
        EliminarNodo(L,aux,ant);
      InformarBrandsen(aux^.datos);
      CalcularIngresantesAno(aux^.datos.fecha,V);
      SumoInscripto(aux^.datos.codigo,VecIns);
      ant := aux;
      aux := aux^.sig;
    End;
...
'Ingrese los datos de los estudiantes (ingrese "." como apellido para finalizar):'
  );
  Repeat
    write('Apellido: ');
    readln(est.apellido);
...
Begin
  ...
  While actual <> Nil Do
    Begin
      ...
      writeln('Fecha: ', actual^.datos.fecha.dia, '/', actual^.datos.fecha.mes,
              '/', actual^.datos.fecha.ano);
